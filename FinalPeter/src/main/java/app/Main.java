        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 5 -> {
                    ...
                    // Бинарный поиск по естественному порядку (id, затем name, затем age)
                    Person[] arr = list.toArray(new Person[0]);
                    // Убедимся, что массив отсортирован в естественном порядке
                    parallelMerge.sort(arr, Comparator.naturalOrder());
                ...
                case 10 -> {
                    // Пересоздать случайные (потоками/стримами)
